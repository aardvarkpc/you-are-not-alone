...
export default function App() {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeCats.length > 0)
      list = list.filter((v) =>
        activeCats.some((c) => (v.category || "").includes(c))
      );
    if (query.trim()) {
  ...
  const shuffleOne = () => {
    ...
    document
      .getElementById(`card-${filtered[idx].id}`)
  ...
  return (
    <div
      ...
      style={{ background: "#f8fafc", color: "#0f172a", padding: "24px" }}
    >
        ...
        >
          <div>
            ...
            <p style={{ fontSize: "13px", opacity: 0.75, margin: 0 }}>
              Affirmations, insights, and real talk—right when you need it.
            </p>
            {loadMsg && (
